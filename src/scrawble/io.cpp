...
terminal_io::terminal_io()
    : pos_(MIN_PLAYER_RACK_COLUMN, PLAYER_RACK_ROW), flags_(FLAG_DIRTY) {
  timeout(-1);
  setlocale(LC_CTYPE, "");
  initscr();
  raw();
  ...
  init_extended_pair(HIGHLIGHT, COLOR_BLACK, COLOR_WHITE);
}
...
void terminal_io::update(game &game) {
  ...
  case KB_UP:
    pos_.y =
        std::max(MIN_BOARD_ROW, pos_.y - (pos_.y == PLAYER_RACK_ROW ? 3 : 2));
    flags_ |= FLAG_DIRTY;
    break;
  case KEY_DOWN:
  case KB_DOWN:
    pos_.y =
        std::min(PLAYER_RACK_ROW, pos_.y + (pos_.y == MAX_BOARD_ROW ? 3 : 2));
    flags_ |= FLAG_DIRTY;
    break;
  case KEY_LEFT:
  case KB_LEFT:
